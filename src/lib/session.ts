...
import db from "@/db";
import { type Session, sessionTable, type User, userTable } from "@/db/schemas";
...
export function generateSessionToken(): string {
  const byptes = new Uint8Array(20);
  crypto.getRandomValues(byptes);

  const token = encodeBase32LowerCaseNoPadding(byptes);
  return token;
}
